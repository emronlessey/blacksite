...
function winGame() {
  ...
  alarm.currentTime = 0;
   ...
   // Tell success page to play sound
...
function resetReactionPuzzle(message) {
  game.reactionReady = false;
  game.reactionDone = false;
...
function renderPuzzleTwo() {
  ...
  if (!game.reactionReady) {
    ...
    setTimeout(function () {
      if (game.step === 1 && game.started && !game.ended && !game.reactionDone && !game.reactionReady) {
        game.reactionReady = true;
        render();

        // Too slow check
        setTimeout(function () {
          if (game.step === 1 && game.started && !game.ended && game.reactionReady && !game.reactionDone) {
            resetReactionPuzzle("Too slow. You missed the signal. Starting over.");
          }
        }, 1000);
      }
    }, waitTime);
  ...
  if (game.reactionReady && !game.reactionDone) {
    box.textContent = "CLICK SUBMIT NOW!";
    gameArea.appendChild(box);
  }
...
document.addEventListener("keydown", function (e) { // hidden key To go straight to success
